...
struct MainDetailPreviewView: View {
    ...
    var body: some View {
        MainDetailView(vm: MainDetailVM(useCase: MainDetailUseCase(repo: MainDetailRepository(networkService: SubwayAPIService())), startVM: startVM), mainVM: MainListVM(useCase: MainListUseCase(repo: MainListRepository(networkStore: SubwayAPIService())), startVM: startVM))
        //        Text("dd")
    }
}
